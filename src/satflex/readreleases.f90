...
subroutine readreleases
  ...
  ! zzpoint             list of heights for multiple level release, if zzpoint *
  !                     zpoint1 and zpoint2 should not be used                 *
...
25  numpoint=numpoint-1
  ...
  if (stat.ne.0) write(*,*)'ERROR: could not allocate zweight'
...
250 close(unitreleases)
  ...
  if (lroot) write (*,*) 'Particles allocated (maxpart)  : ',config%maxpart
  ...
  if (releaserate.gt. &
       0.99*real(config%maxpart)/real(lage(nageclass))) then
    if (numpartmax > config%maxpart.and.lroot) then
      write(*,*) '#####################################################'
      write(*,*) '#### FLEXPART MODEL SUBROUTINE READRELEASES:     ####'
      write(*,*) '####                                             ####'
      ...
      write(*,*) 'Maximum allowed release rate is: ', &
           real(config%maxpart)/real(lage(nageclass)),' particles per second'
      write(*,*) &
           'Total number of particles released during the simulation is: ', &
           numpartmax
      write(*,*) 'Maximum allowed number of particles is: ',config%maxpart
    endif
  endif
