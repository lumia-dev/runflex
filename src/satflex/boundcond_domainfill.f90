...
subroutine boundcond_domainfill(itime,loutend)
    ...
    do i=1,numpart
        ...
        if (pp%active) then
            if ((ytra1(i) .gt. real(ny_sn(2))) .or. (ytra1(i) .lt. real(ny_sn(1)))) pp%active = .false.
            if ((.not. xglobal) .or. (nx_we(2) .ne. (nx - 2))) then
                if ((xtra1(i) .lt. real(nx_we(1))) .or. (xtra1(i) .gt. real(nx_we(2)))) then
                    pp%active = .false.
                end if
            endif
        end if
    ...
    numactiveparticles = count(particles%active)
                    ...
                    do ipart = minpart, config%maxpart
                        ...
                        if (pp%free) then
                            ...
                            if (((ztra1(ipart).gt.3000.).and. &
                                ...
                                pp%t = itime
                                itramem(ipart) = pp%t
                                itrasplit(ipart) = pp%t + ldirect * itsplit
...
171                 continue
    ...
    xm=0.
    do i=1,numpart
        if (particles(i)%active) xm=xm+xmass1(i,1)
    end do
