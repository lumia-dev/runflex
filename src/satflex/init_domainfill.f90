...
subroutine init_domainfill
    ...
    real :: gridarea(0:nymax-1),pres(nzmax),ylat,ylatp,ylatm,hzone
    ...
    do jy=ny_sn(1),ny_sn(2)          ! loop about latitudes
        do ix=nx_we(1),nx_we(2)      ! loop about longitudes
            pres(1)=rho(ix,jy,1,1)*r_air*tt(ix,jy,1,1)
            pres(nz)=rho(ix,jy,nz,1)*r_air*tt(ix,jy,nz,1)
            colmass(ix,jy)=(pres(1)-pres(nz))/ga*gridarea(jy)
    ...
    do jy=ny_sn(1),ny_sn(2)      ! loop about latitudes
        ...
        do ix=nx_we(1),nx_we(2)      ! loop about longitudes
            ...
            if (ncolumn /= 0) then
                ...
                do kz=1,nz
                    pres(kz)=rho(ix,jy,kz,1)*r_air*tt(ix,jy,kz,1)
                end do


                deltacol=(pres(1)-pres(nz))/real(ncolumn)
                pnew=pres(1)+deltacol/2.
                jj=0
                do j=1,ncolumn
                    jj=jj+1
                    ...
                    if (ncolumn.gt.20) then
                        ...
                        pnew=pres(1)-ran1(idummy)*(pres(1)-pres(nz))
                    endif

                    do kz=1,nz-1
                        if ((pres(kz).ge.pnew).and.(pres(kz+1).lt.pnew)) then
                            dz1=pres(kz)-pnew
                            dz2=pnew-pres(kz+1)
                            dz=1./(dz1+dz2)
                            ...
                            if (ipin.eq.0) then
                                ...
                                if (((ztra1(numpart+jj).gt.3000.).and. &
                                    ...
                                    itrasplit(numpart + jj) = pp%t + ldirect * itsplit
    ...
    do j=1,numpart
        pp => particles(j)
        if ((xtra1(j) < 0.) .or. (xtra1(j) >= real(nxmin1)) .or. (ytra1(j) < 0.) .or. (ytra1(j) >= real(nymin1))) then
            pp%active = .false.
        endif
        nullify(pp)
    end do
